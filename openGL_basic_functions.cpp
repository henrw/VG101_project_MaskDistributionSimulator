...
void ShowDialog()
{
    if (dialog_city_index >= 0)
    {
        ...
        DrawString(total_num_str, x_origin - 60, y_end + 10, GLUT_BITMAP_HELVETICA_12);
        ...
        for (int i = 0; i < history_inf_cache_count && i < city_info.history_inf_nums.size(); i++)
        {
            ...
            int y = y_origin - (float)(y_origin - y_end - 5) / city_info.total_num * city_info.history_inf_nums[i];
            y = max(y, y_end);
...
void DrawRoadText()
{
    //    DrawString2("10000", 222, 167, 20,GLUT_STROKE_ROMAN);//A
    //    DrawString2("20000", 400, 135, 20,GLUT_STROKE_ROMAN);//C
    //    DrawString2("22000", 324, 298, 70,GLUT_STROKE_ROMAN);//D
    //    DrawString2("20000", 396, 190, -85,GLUT_STROKE_ROMAN);//E
    //    DrawString2("20000", 435, 175, -40,GLUT_STROKE_ROMAN);//F
    //    DrawString2("30000", 195, 277, 55,GLUT_STROKE_ROMAN);//G
    //    DrawString2("22000", 381, 290, -90,GLUT_STROKE_ROMAN);//H
}
